...
class SendModal extends Component {
  ...
  pay(user, coin, quantity) {
    ...
    } else if (quantity > this.props.validCoins[coin].quantity) {
      return this.setState({
        errorMessage: 'You do not have enough coins',
      });
    } else if (quantity <= 0) {
      return this.setState({
    ...
    return this.props.pay(user, coin, quantity);
  }
      ...
      <Modal open={this.props.state} onClose={() => this.props.onCloseModal()} little styles={{ modal: { backgroundColor: 'rgb(14, 39, 89)', borderRadius: '10px', minWidth: '300px' } }} >
        ...
        <div className="TransferCoins-form">
          ...
          <button
            ...
            onClick={() => {
              this.pay(this.state.selectedUser, this.state.selectedCoin, this.state.quantity);
            }}
          >
            Pay
          </button>
